...
public class StringUtil
{
	...
	public static String encodeURIComponent(String s, String allow) {
        if (s == null) {
            return null;
        }
        ...
        while (current < oldLength) {
            ...
            while (nextToEncode < oldLength
                    && isAllowed(s.charAt(nextToEncode))) {
                nextToEncode++;
            }
            ...
            while (nextAllowed < oldLength
                    && !isAllowed(s.charAt(nextAllowed))) {
                nextAllowed++;
            }
            ...
            try {
                ...
                for (int i = 0; i < bytesLength; i++) {
                    encoded.append('%');
                    encoded.append(hexChar[(bytes[i] & 0xf0) >> 4]);
                    encoded.append(hexChar[bytes[i] & 0xf]);
                }
